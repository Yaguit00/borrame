...
#original
#original    def test_empty_dir(self):
#original        tmp_dir_name = mkdtemp(prefix=' with spaces ')
#original
#original        try:
#original            self.child = pexpect.spawn('/bin/bash "{}"'.format(self.script_name), encoding='utf-8')
#original            self.assertFalse(self.child.expect_exact('Introduzca el nombre de un directorio: '))
#original        except:
#original            self.assertTrue(False)
#original
#original        try:
#original            self.child.sendline(tmp_dir_name)
#original            self.assertFalse(self.child.expect('El numero de ficheros y directorios en {} es de 0 y 0, respectivamente'.format(tmp_dir_name)))
#original        except:
#original            self.assertTrue(False)
#original        self.assertTrue(True)
#original
#original        os.rmdir(tmp_dir_name)
#original        self.child.terminate(force=True)
#original
